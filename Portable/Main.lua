--[[ ==========================================================================
	...
	Portable
		Quick Access Buttons for Mage Portal and Teleport Spells.
		
		By Feyawen-Llane
			...for my wife, Minxx-Llane
		
		
		Version 4.4.0
			+ Minimap Button via Request
			+ Data Broker with Drop Down Spell Access
			+ Toolbar Sizes, Full, Half, and Off
			* Cleaned up the Library folder to remove redundant files
			
		Version 4.1.0
			+ Fixed ID's in Options to make them Unique
			+ Fixed Show Options helper to use Title instead of Name
			
		Version 4.0.0
			+ Initial Public Release, After Much Convincing by Minxx
			+ Fixed some rough edges of the UI
			+ Re-made image sets round & square (so the old cities are in the newer graphics)
			+ Added Spell Order Learning (Where you go most, is given higher priority)
			+ Added Drag and Drop Manual Spell Ordering (Put them in the order you want them in)
		
		Version 3.2.0
			+ Added Hearthstone / Garrison Hearthstone Support
			+ Removed Dock Style Layout, nobody in the guild uses it and it was a bit wonky
			+ Removed Poster Style Image Set, they look stupid in any Layout besides Dock Style
			+ Added "Center of Attention" Layout
			+ Added "Look at Me!" Layout, Left and Right
			+ Added "Priority" RIGHT, so now it has a Left and Right Layout
		
		Version 3.0.5
			+ Fixed a Bug with ACE Config, Still Learning
		
		Version 3.0.0
			+ Another Complete Rewrite, Built a "Template" for starting a new AddOn with ACE Config
			+ Language Support
			+ Now using ACE Libraries for Options and Configuration UI
			+ Poster Image Set
			+ Dock Style Layout
			+ Support for Stormshield and Warspear
			+ Rearranged the Priority Layout for "Warlords of Dreanor"
			
		Version 2.0.5
			+ Oops, Support for "Old Dalaran" (Apparently Not All Mages Know About This)
			
		Version 2.0.0
			+ Complete Rewrite, OMG I Can't See Straight!
			+ Fixed All the New Bugs Added by Blizzard Changing Things on Me, Thanks Blizzard!
			+ Support for Vale of Eternal Blossoms
			+ Rearranged the Priority Layout for "Mist of Pandaria"
			+ Guild-mates and Friends Can Now Enjoy Portable for Themselves
			
		Version 1.5.0
			+ Support for Tol Barad
			+ Rearranged the Priority Layout for "Cataclysm" (To Stormwind and Orgrimmar)
...
function me:UpdateUI_ButtonGrid()
	...
	elseif (me.db.profile.iconLayout == 4) then
		...
		me.ui.button16:SetPoint("TOPLEFT", me.ui.button12, "TOPRIGHT", 0, -space)	
	...
	elseif (me.db.profile.iconLayout == 5) then
		...
		me.ui.button16:SetPoint("TOPRIGHT", me.ui.button12, "TOPLEFT", 0, -space)		
	...
	elseif (me.db.profile.iconLayout == 6) then
		...
		me.ui.button16:SetPoint("TOPRIGHT", me.ui.button8, "TOPLEFT", -space, 0)
	-- Invalid
	else
